...
function Flatpickr(element, config) {
	...
	function setHoursFromInputs() {
		...
		if (self.amPM) hours = hours % 12 + 12 * (self.amPM.innerHTML === "下午");
	...
	function setHours(hours, minutes, seconds) {
		...
		if (!self.config.time_24hr && self.selectedDates.length) self.amPM.textContent = self.latestSelectedDateObj.getHours() >= 12 ? "下午" : "上午";
	...
	function timeWrapper(e) {
		...
		if (self.amPM && e.target === self.amPM) return e.target.textContent = ["上午", "下午"][e.target.textContent === "上午" | 0];
		...
		if (e.type !== "input" || e.target.value.length === 2) {
			...
			if (self.amPM && e.target === self.hourElement && (step === 1 ? newValue + curValue === 23 : Math.abs(newValue - curValue) > step)) self.amPM.textContent = self.amPM.innerHTML === "下午" ? "上午" : "下午";
...
Flatpickr.l10ns.default = Object.create(Flatpickr.l10ns.zh);
